	...
	class AppMgr
	{
		...
		public static void Main(string[] strArgs)
		{
			...
			catch (Exception)
			{
				...
				MessageBox.Show("TBEST has stopped working. Please restart the application. If the problem persists, please contact TBEST Technical Support.", "TBEST Ridership Estimation Model Run",MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
			}
			finally
			{
				Environment.Exit(0);
		...
		private readonly frmTBESTStandAloneModelRun m_mainForm;
		private AppMgr()
		{
			m_mainForm = new frmTBESTStandAloneModelRun();
		...
		public async Task StartAsync(string[] strArgs)
		{
			...
			if (TBESTApplicationFunctions.mTransitSystem is not null & TBESTApplicationFunctions.mScenario is null)
			{
				m_mainForm.Text = "TBEST Ridership Estimation Batch Model";
			}
			m_mainForm.Show();
		}
		...
		private static string GetTBESTInstallLocation()
		{
			try
			{
				...
				pRegKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32);
